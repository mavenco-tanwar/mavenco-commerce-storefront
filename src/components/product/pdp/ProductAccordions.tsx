...
import { NormalizedProduct, PdpSectionItem } from "@/types/pdp-template.types";
...
export function ProductAccordions({ product, sections, className = "" }: ProductAccordionsProps) {
  ...
  const isDescEnabled = !sections || sections.find((s) => s.id === 'sec_overview' || s.id === 'sec_tabs' || s.type === 'tabs' || s.type === 'description')?.enabled !== false;
  const isSpecEnabled = !sections || sections.find((s) => s.id === 'sec_specs' || s.type === 'specifications')?.enabled !== false;
  const isFabricEnabled = !sections || sections.find((s) => s.id === 'sec_care' || s.type === 'careInstructions')?.enabled !== false;
  const isShippingEnabled = !sections || sections.find((s) => s.id === 'sec_shipping' || s.type === 'shipping')?.enabled !== false;

  const hasDescription = Boolean(product.description && product.description.trim()) && isDescEnabled;
  const hasFeatures = Boolean(product.features && product.features.length > 0);
  const hasCare = Boolean(product.careInstructions && product.careInstructions.length > 0) && isFabricEnabled;
