...
export default async function ContactPage(props: {
  ...
  const showStores = sec.stores !== false;
  const showForm = sec.form !== false;
      ...
      <main className="flex-1 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16 space-y-12 w-full">
        ...
        {(showStores || showForm) && (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            {/* 2. Physical Stores / Salons Section */}
            {showStores && (
              <div className={`${showForm ? 'lg:col-span-6' : 'lg:col-span-12'} space-y-4`}>
                ...
                <div className="space-y-4">
                  {config.stores?.map((store, idx) => (
                    <div
                      key={idx}
                      className="p-6 rounded-3xl border transition-all space-y-4 overflow-hidden"
                      style={{
            ...
            {/* 3. Concierge Inquiry Form Section */}
            {showForm && (
              <div
                className={`${showStores ? 'lg:col-span-6' : 'lg:col-span-12'} p-8 rounded-3xl border shadow-xl space-y-6`}
                style={{
          ...
          config.customSections
            ...
            ?.map((customSec) => (
              <section
                ...
                style={{
                  backgroundColor: customSec.backgroundColor || d.formCardBg || '#101320',
                  borderColor: d.formCardBorder || 'rgba(255,255,255,0.08)',
                }}
              >
                {customSec.title && (
                  <h2
