...
export async function POST(
  ...
  try {
    ...
    const restored = await BuilderPageService.restoreVersion(tenantId, id, versionNum);

    try {
      revalidatePath(`/${restored.slug}`);
      revalidatePath(`/stores/${tenantId}/${restored.slug}`);
    } catch {}

    return NextResponse.json(
      {
        success: true,
        message: `Restored draft to version ${versionNum}`,
        data: restored,
      },
      { headers: corsHeaders() }
    );
