...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    console.warn('Error loading theme versions:', err);
  }
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const targetVersion = await db.collection('theme_versions').findOne({
      tenantId: tenantSlug,
      version: Number(version),
    });

    if (!targetVersion || !targetVersion.theme) {
      return NextResponse.json(
