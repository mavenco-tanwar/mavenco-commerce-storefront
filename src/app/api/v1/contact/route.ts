...
const transporter = nodemailer.createTransport({
  ...
  auth: {
    user: process.env.SMTP_USER || 'ammar.tanwar.dev@gmail.com',
    pass: (process.env.SMTP_PASS || 'ubup gwkg sbeo bldb').replace(/\s+/g, ''), // App password
  },
});
...
export async function POST(req: Request) {
  try {
    ...
    // 1. Save directly into MongoDB Atlas `contact_inquiries`
    try {
    ...
    // 2. Dispatch formatted Notification Email directly to ammar.tanwar.dev@gmail.com
    ...
    await transporter.sendMail({
      from: `"Mavenco Lead Engine" <${process.env.SMTP_USER || 'ammar.tanwar.dev@gmail.com'}>`,
      to: 'ammar.tanwar.dev@gmail.com',
      replyTo: cleanEmail,
      subject: `🔥 [Demo Request] ${cleanBrand} (${cleanName}) - ${cleanPlan}`,
      html: notificationHtml,
    });

    // 3. Send Auto-Acknowledgement Email to the prospect
    try {
      const ackHtml = `
        <!DOCTYPE html>
        <html>
        ...
        <body>
          <div class="container">
            ...
            <div class="footer">
              Mavenco Commerce Cloud • Ammar Tanwar • ammar.tanwar.dev@gmail.com
            </div>
          </div>
        </body>
        </html>
      ...
      await transporter.sendMail({
        from: `"Mavenco Commerce Team" <${process.env.SMTP_USER || 'ammar.tanwar.dev@gmail.com'}>`,
        to: cleanEmail,
    ...
    return NextResponse.json({
      success: true,
      message: 'Demo inquiry dispatched successfully to ammar.tanwar.dev@gmail.com and WhatsApp +918239019096.',
